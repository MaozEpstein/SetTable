      ...
      >
        ...
        {categories.map((cat) => (
          <Pill
            ...
            onLongPress={
              cat.isCustom && onLongPressCategory
                ? () => onLongPressCategory(cat)
                : undefined
            }
            emoji={cat.emoji}
            label={cat.label}
            count={countsByCategory?.[cat.key] ?? 0}
          />
        ))}
        {onAddCategory && (
          <Pressable
...
function Pill({ isActive, onPress, onLongPress, emoji, label, count }: PillProps) {
  return (
    <Pressable
      onPress={onPress}
...
const styles = StyleSheet.create({
  ...
  labelActive: { color: '#FFFFFF', fontFamily: fontFamily.bold },
  badge: {
    minWidth: 22,
    height: 22,
    borderRadius: 11,
